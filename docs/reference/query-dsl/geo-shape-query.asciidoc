...
An example of when to use this would be checking if a road (mapped as a 
`polygon`) overlaps with a city (mapped as a `polygon`); The two shapes
would intersect if:
1. the boundary of the road overlaps with the boundary of the city
2. the interior of the road overlaps with the boundary of the city
3. the boundary of the road overlaps with the interior of the city
4. the interior of the road overlaps with the interior of the city
...
Given a query with geometry representing a suburb (as a `polygon`), return
all documents whose `geo_shape` field is wholly within the suburb.
* `CONTAINS` - Return all documents whose `geo_shape` field
contains the query geometry.
Given a query with geometry representing an address (as a `point`), return
all documents whose `geo_shape` field wholly contains the address.
